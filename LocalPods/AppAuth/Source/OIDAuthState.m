...
- (void)withFreshTokensPerformAction:(OIDAuthStateAction)action {
    [self withFreshTokensPerformAction:action additionalParams:nil];
}

- (void)withFreshTokensPerformAction:(OIDAuthStateAction)action additionalParams:(NSDictionary<NSString *, NSString*> *)params {
    if (!_refreshToken) {
        [OIDErrorUtilities raiseException:kRefreshTokenRequestException];
    }
    ...
    if ([self.accessTokenExpirationDate timeIntervalSinceNow] > kExpiryTimeTolerance
        ...
        // refresh the tokens
        OIDTokenRequest *tokenRefreshRequest = [self tokenRefreshRequestWithAdditionalParameters:params];
